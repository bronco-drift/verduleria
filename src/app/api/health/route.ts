...
export async function GET() {
  const env = {
    ...
    urlHost: process.env.NEXT_PUBLIC_SUPABASE_URL?.match(
      /https:\/\/([^.]+)/
    )?.[1] ?? null,
    hasAnonKey: !!process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY,
    anonKeyPrefix:
      process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY?.slice(0, 18) ?? null,
    hasServiceRole: !!process.env.SUPABASE_SERVICE_ROLE_KEY,
  };

  let query: { success: boolean; error: string | null; storeCount: number } = {
    success: false,
    error: null,
    storeCount: 0,
  };
