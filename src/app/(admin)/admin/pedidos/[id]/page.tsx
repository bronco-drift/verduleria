...
export default async function OrderDetailPage({
  ...
  const driverList = (drivers ?? []).map((d) => {
    const row = d as unknown as DriverRow;
    return {
      id: row.user_id,
      ...
      <header className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-semibold">
            Pedido <span className="font-mono text-base text-muted-foreground">#{order.id.slice(0, 8)}</span>
          </h1>
          ...
          <Card className="p-4 space-y-2">
            ...
            {order.customer_notes && (
              <p className="text-sm bg-muted p-2 rounded mt-2">
                <span className="font-medium">Notas:</span> {order.customer_notes}
              </p>
            )}
          </Card>
